...
- One-click Play switching of the default Cursor instance: precise SQLite injection, verified default-profile close/restart with `--new-window`, path recovery and restricted UAC retry on Windows (unsaved work may be lost).
...
- 高密度额度卡（含 Bot 周期内全模型消费金额）、支持网页登录、裸 Access Token、单行网页 Token、Cockpit JSON、JSON 文件及本机只读导入的统一添加入口、Play 一键切号（覆盖默认 Cursor 登录状态并强制关闭/重启默认实例；Windows 下未保存内容可能丢失）、存盘恢复、筛选分页和完整敏感导出。
