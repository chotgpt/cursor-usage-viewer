...
export function SettingsPage({ language, setLanguage, theme, setTheme, updater }: {
  ...
  const [cursorPathBusy, setCursorPathBusy] = useState(false);
  ...
  const loadCursorSettings = async () => { setCursorSettingsLoading(true); setCursorSettingsError(""); try { const value = await cursorService.getSettings(); const preset = [-1, 2, 5, 10, 15].includes(value.autoRefreshMinutes); setCursorSettings(value); setRefreshChoice(preset ? String(value.autoRefreshMinutes) : "custom"); if (!preset) setCustomRefreshMinutes(String(value.autoRefreshMinutes)); setCursorAppPath((await cursorService.getCursorAppPath()) ?? ""); } catch (error) { setCursorSettings(null); setCursorSettingsError(String(error)); } finally { setCursorSettingsLoading(false); } };
  ...
  const saveCursorPath = async (path: string) => { setCursorPathBusy(true); setCursorSettingsError(""); try { setCursorAppPath((await cursorService.saveCursorAppPath(path)) ?? ""); } catch (error) { setCursorSettingsError(String(error)); } finally { setCursorPathBusy(false); } };
  const pickCursorPath = async () => { const path = await open({ multiple: false, directory: false }); if (typeof path === "string") await saveCursorPath(path); };
  const detectCursorPath = async () => { setCursorPathBusy(true); setCursorSettingsError(""); try { setCursorAppPath((await cursorService.detectCursorAppPath(true)) ?? ""); } catch (error) { setCursorSettingsError(String(error)); } finally { setCursorPathBusy(false); } };
    ...
    <div className="settings-container"><div className="settings-content" id={`settings-${activeTab}-panel`} role="tabpanel" aria-labelledby={`settings-${activeTab}-tab`}>
      {activeTab === "general" ? <>
        ...
        <div className="group-title">{l("Cursor 额度", "Cursor usage")}</div><div className="settings-group">
          ...
          <SettingRow title={l("Cursor 路径", "Cursor path")} description={l("留空则使用默认路径", "Leave empty to use the default path")}><div className="cursor-path-control"><input className="cursor-path-input" aria-label={l("Cursor 路径", "Cursor path")} placeholder={l("默认路径", "Default path")} value={cursorAppPath} disabled={cursorSettingsLoading || cursorPathBusy} onChange={(event) => setCursorAppPath(event.target.value)} onBlur={() => void saveCursorPath(cursorAppPath)}/><button className="btn btn-secondary" disabled={cursorSettingsLoading || cursorPathBusy} onClick={() => void pickCursorPath()}>{l("选择", "Browse")}</button><button className="btn btn-secondary" disabled={cursorSettingsLoading || cursorPathBusy} onClick={() => void detectCursorPath()}><RefreshCw size={16} className={cursorPathBusy ? "spin" : undefined}/>{cursorPathBusy ? l("加载中...", "Loading...") : l("自动检测", "Auto detect")}</button></div></SettingRow>
        </div>
