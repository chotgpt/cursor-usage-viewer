...
type ExportState = { json: string; ids: string[]; revealed: boolean; copied: boolean; webTokenCopied: boolean; webTokenNotice: AppMessage | null; pathCopied: boolean; saving: boolean; savedPath: string };
...
export default function App() {
  ...
  const [addOperation, setAddOperation] = useState<"oauth" | "import" | null>(null);
  ...
  // Feedback stays inside the dialog: the global message bar is hidden behind the modal overlay.
  async function copyWebTokens() { if (!exportState) return; const { tokens, skipped } = webSessionTokensFromExport(exportState.json); const notice = (value: AppMessage | null) => setExportState((state) => state ? { ...state, webTokenNotice: value } : state); if (!tokens.length) { notice({ text: l("没有可转换的账号：缺少 Access Token 或无法解析出 user_ 开头的用户 ID", "No convertible accounts: missing access token or no user_ ID could be parsed"), tone: "error" }); return; } try { await navigator.clipboard.writeText(tokens.join("\n")); setExportState((value) => value ? { ...value, webTokenCopied: true, webTokenNotice: skipped ? { text: language === "en" ? `Copied ${tokens.length} web token(s); ${skipped} account(s) skipped because no access token or user_ ID could be resolved` : `已复制 ${tokens.length} 个网页 Token，${skipped} 个账号因缺少 Access Token 或无法解析用户 ID 被跳过`, tone: "warning" } : null } : value); window.setTimeout(() => setExportState((value) => value ? { ...value, webTokenCopied: false } : value), 1200); } catch (error) { notice({ text: `${l("复制网页 Token 失败", "Failed to copy web token")}: ${readableForLanguage(error, language)}`, tone: "error" }); } }
      ...
      {page === "settings" ? <SettingsPage language={language} setLanguage={setLanguage} theme={theme} setTheme={setTheme} updater={updater} /> : <>
        ...
        <section className="toolbar">
          ...
          <div className="toolbar-right">
            <button className="btn btn-primary icon-only" title={l("添加账号", "Add account")} aria-label={l("添加账号", "Add account")} onClick={() => setShowAddAccount(true)}><Plus size={14}/></button>
            <button className="btn btn-secondary icon-only" title={t.refreshAll} aria-label={t.refreshAll} aria-busy={busy.has("refresh-all")} onClick={() => void refreshMany(accounts.map((item) => item.id))} disabled={!accounts.length || busy.has("refresh-all")}><RefreshCw size={14} className={busy.has("refresh-all") ? "loading-spinner" : ""}/></button>
            ...
            <button className="btn btn-secondary icon-only" title={t.settings} aria-label={t.settings} onClick={() => setPage("settings")}><SettingsIcon size={14}/></button>
          </div>
        </section>
        {filtered.length > 0 && (
