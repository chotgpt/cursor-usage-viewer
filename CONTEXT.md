...
- **自动刷新**：用户开启 Cursor 专用刷新间隔后，由 Tauri 应用进程按周期调用与手动刷新相同的后端链路；默认 10 分钟，可关闭，窗口隐藏到托盘后继续运行。
- **网页登录**：用户从添加账号弹层主动启动的 Cursor 固定 device flow；只打开 `loginDeepControl` 并轮询 `auth/poll`，可取消且最多等待 300 秒。
...
- **系统托盘**：主窗口隐藏后仍承载应用进程、应用更新检查及用户已启用的 Cursor 自动刷新；退出应用会停止这些任务。
...
- 手动与自动刷新共用 Cockpit 的 Token 续期规则：Access Token 无法解析或在五分钟内到期时，使用 Refresh Token 请求 Cursor OAuth Token 端点；成功后保存新 Token，失败时继续尝试现有 Access Token。只有自动刷新间隔大于 0 且账号任务到期时，后台链路才可续期和查询额度。
...
- 默认关闭窗口时询问最小化到托盘或退出；托盘内继续应用更新检查，并在 Cursor 自动刷新间隔大于 0 时继续额度刷新。
